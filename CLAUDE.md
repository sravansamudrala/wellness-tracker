...
- **Fully wired to the backend:** `Skincare.tsx` (loads/saves today's routine; persists on every checkbox toggle) and `Settings.tsx` (reminder times + notifications toggle).
- **Stubs — return a bare heading:** `Food.tsx`, `Water.tsx`, `Weight.tsx`, `History.tsx`.
